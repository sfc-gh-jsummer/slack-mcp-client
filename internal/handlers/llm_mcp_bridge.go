...
func (b *LLMMCPBridge) executeToolCall(ctx context.Context, toolCall *ToolCall, extraArgs map[string]interface{}) (string, error) {
	// Filter out Slack-specific arguments that shouldn't be passed to MCP tools
	slackSpecificArgs := map[string]bool{
		"channel_id": true,
		"thread_ts":  true,
	}
	
	for k, v := range extraArgs {
		// Skip Slack-specific arguments - they're for internal context only
		if slackSpecificArgs[k] {
			continue
		}
		// Add any other extra arguments to the tool call args
